...
const Stats = () => {
  ...
  useEffect(() => {
    ...
    const doctorDataFetch = async () => {
      const data = await (
        await fetch(
          "https://medicinebd-medicine-details-server.vercel.app/v1/doctors/0/9"
        )
      ).json();
  ...
  useEffect(() => {
    ...
    const hospitalDataFetch = async () => {
      const data = await (
        await fetch(
          "https://medicinebd-medicine-details-server.vercel.app/v1/hospitals/0/9"
        )
      ).json();

      // set state when the data received
      setTotalHospitals(data);
      setLoading(false);
    };
