...
export default function AutoCompleteHospitals() {
  ...
  const handleInputChange = async (e) => {
    ...
    let timeout = setTimeout(() => {
      ...
      axios
        .get(
          `https://medicinebd-medicine-details-server.vercel.app/v1/hospitals/search?name=${e.target.value}`
        )
        .then((res) => {
                ...
                <>
                  {searchResults.map((searchResult) => (
                    <ul className="p-3 my-2 border border-indigo-600 bg-base-100 rounded-box">
                      ...
                      <p className="text-center">{searchResult.address}</p>

                      <Link
                        onClick={handleOnclick}
                        className="btn btn-sm my-2"
                        href={`/hospital/${searchResult.id}`}
                      >
                        Details
                      </Link>
                    </ul>
